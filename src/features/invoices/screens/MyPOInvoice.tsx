...
export default function MyPOInvoice({ navigation }) {
  const { colors } = useTheme();
  const styles = makeStyles(colors);
  ...
  const fetchMyPOInvoices = async (
    appliedFilters = filters,
    pageNumber = 1,
    isLoadMore = false,
  ) => {
    try {
      const res = await api.post('/invoice/getAllByUserId', {
        limit,
      ...
      console.log('Fetch My PO Invoice Error:', err?.response || err);
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const loadMore = () => {
    ...
    setPage(nextPage);
    setLoadingMore(true);
    fetchMyPOInvoices(filters, nextPage, true);
  };
  ...
  const resetFilters = () => {
    ...
    setPage(1);
    setHasMore(true);
    fetchMyPOInvoices(emptyFilters, 1);
  };
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.center}>
            <Text style={styles.empty}>No PO Invoices found for you.</Text>
          </View>
        }
