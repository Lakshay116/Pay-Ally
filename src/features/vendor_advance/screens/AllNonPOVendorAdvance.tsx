...
import {
  fetchAllNonPOVendorAdvances,
  fetchVendorAdvanceReferenceNumbers,
  fetchEntities,
  fetchOffices,
  fetchDepartments,
  fetchVendors,
...
export default function AllNonPOVendorAdvance({ navigation }) {
  ...
  const loadAdvances = useCallback(
    async (appliedFilters = filters, pageNumber = 1, isLoadMore = false) => {
      try {
        ...
        const rows = await fetchAllNonPOVendorAdvances(
          appliedFilters,
          PAGE_SIZE,
        ...
        console.log(
          'Fetch All Non-PO Vendor Advance Error:',
          err?.response || err,
        );
  ...
  const loadInitialFilterOptions = useCallback(async () => {
    try {
      ...
      console.log('Initial vendor advance filter load error:', err);
    }
  }, []);
  ...
  const searchAdvanceReferences = async text => {
    try {
      const data = await fetchVendorAdvanceReferenceNumbers(text);
