...
const PAGE_SIZE = 10;
const DEFAULT_PAGINATION = { limit: PAGE_SIZE, page: 1 };
...
const getCreatedByName = item => {
  const fullName = [item?.Updater?.FirstName, item?.Updater?.LastName]
    .filter(Boolean)
    .join(' ')
    .trim();

  return fullName || item?.Updater?.UserName || '-';
};
...
const getTotalEstimatedSpend = item => {
  ...
  const currencySymbol = item?.PRCurrency?.CurrencySymbol || '₹';
...
const getRFQStatus = item => {
  return item?.PurchaseRequestRFQs?.length > 0 ? 'Raised' : 'Pending';
};

export default function RFQList({ navigation, type = 'all' }) {
  const isMyRequests = type === 'my';
  const endpoint = isMyRequests
    ? '/purchaseRequest/getApprovedByUserIdForRFQ'
    : '/purchaseRequest/getApprovedForRFQ';
    ...
    async (appliedFilters = filters, pageNumber = 1, isLoadMore = false) => {
      try {
        ...
        const rows = await fetchList(
          appliedFilters,
          appliedFilters.pagination?.limit || PAGE_SIZE,
          (pageNumber - 1) * (appliedFilters.pagination?.limit || PAGE_SIZE),
        );
        ...
        console.log('Fetch RFQ Error:', err?.response || err);
  ...
  const loadInitialFilterOptions = useCallback(async () => {
    try {
      ...
      console.log('Initial RFQ filter load error:', err);
    }
  }, [endpoint]);
  ...
  const searchRequestNumbers = async text => {
    try {
      const data = await fetchRFQNumbers(endpoint, text);
      setRequestNumbers(data);
    } catch (err) {
      console.log('RFQ number fetch error:', err);
    }
  };

  const searchEntity = async text => {
    try {
      const data = await fetchRFQEntities(text);
      setEntities(data);
    } catch (err) {
  ...
  const searchOffice = async text => {
    try {
      const data = await fetchRFQOffices(text);
      setOffices(data);
    } catch (err) {
  ...
  const searchDepartment = async text => {
    try {
      const data = await fetchRFQDepartments(text);
      setDepartments(data);
    } catch (err) {
  ...
  const searchUsers = async text => {
    try {
      const data = await fetchRFQUsers(text);
      setUsers(data);
    } catch (err) {
  ...
  const renderFilterSelect = (
    ...
    <View style={styles.filterField}>
      ...
      <MultiSelect
        ...
        value={Array.isArray(filters[valueKey]) ? filters[valueKey] : (filters[valueKey] ? [filters[valueKey]] : [])}
        onChange={items => setFilters(prev => ({ ...prev, [valueKey]: items.length ? items : null }))}
        ...
        renderRightIcon={() =>
          (Array.isArray(filters[valueKey]) ? filters[valueKey].length : filters[valueKey]) ? (
            <TouchableOpacity
              onPress={() => setFilters(prev => ({ ...prev, [valueKey]: null }))}
            >
  ...
  const renderListCard = item => {
    const statusText = getRFQStatus(item);
    const statusColors = getStatusColors(statusText, colors);
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() =>
          navigation.navigate('RFQDetails', {
            rfq: item,
          })
        }
      >
